...
function eliminarIntegrante(cardNumber){
    //var cardTitle = document.querySelector('.card:nth-child(' + cardNumber + ') .card-title');
    //alert("El título de la tarjeta " + cardNumber + " es: " + integrante[cardNumber].name );
    integrante.splice(cardNumber, 1);
    localStorage.setItem("array", JSON.stringify(integrante));
    //for (let i = 0; i < integrante.length; i++) {
    //        alert(integrante[i].name + ' ' + integrante[i].email);
    //    }
